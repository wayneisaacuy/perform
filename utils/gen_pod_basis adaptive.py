...
windowSize = 200 # there is a 1 added later

iter_start = 0  # zero-indexed starting index for snapshot array
iter_end = windowSize #4000  # zero-indexed ending index for snapshot array
iter_skip = 1
...
def main():
    ...
    for group_idx, var_idx_list in enumerate(var_idxs):
        ...
        # save data to disk
        cent_file = os.path.join(out_dir, "cent_prof_cons")
        norm_sub_file = os.path.join(out_dir, "norm_sub_prof_cons")
        norm_fac_file = os.path.join(out_dir, "norm_fac_prof_cons")
        spatial_mode_file = os.path.join(out_dir, "spatial_modes_cons")
        sing_vals_file = os.path.join(out_dir, "singular_values_cons")
